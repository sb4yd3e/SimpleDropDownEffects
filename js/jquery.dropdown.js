    ...
    $.DropDown.prototype = {
        ...
        _initEvents: function () {
            ...
            this.opts.on('click.dropdown', function () {
                if (self.opened && animated) {
                    var opt = $(this);
                    animated = false;
                    self.options.onOptionSelect(opt);
                    self.inputEl.val(opt.data('value'));
                    self.selectlabel.html(opt.html().replace(/<\/?[^>]+(>|$)/g, ""));
                    self.close();
                }
            });

            $(document).on('click', function (e) {
                if ($(e.target).attr('id') !== self.selectlabel[0].id && self.opened && animated) {
                    animated = false;
                    self.close();
                }
            });
